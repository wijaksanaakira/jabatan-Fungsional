...
<div class="space-y-6 max-w-3xl mx-auto">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-900">Edit Jabatan Fungsional</h1>
            <p class="mt-1 text-sm text-slate-500">Perbarui informasi jabatan fungsional.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 sm:p-8">
        <form action="{{ route('jabatan.update', $jabatan->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="nama" id="nama" value="{{ old('nama', $jabatan->nama) }}" required
            ...
            <div>
                ...
                <textarea name="keterangan" id="keterangan" rows="3"
                    class="mt-1 block w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-600 focus:border-transparent @error('keterangan') border-red-500 @enderror">{{ old('keterangan', $jabatan->keterangan) }}</textarea>
                @error('keterangan')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
